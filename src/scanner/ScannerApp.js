...
import { FrameGrabber } from './capture/FrameGrabber.js';
import { ScannerUIController } from './ScannerUIController.js';
import { PeerScannerManager } from './PeerScannerManager.js';

/**
 * ScannerApp.js
 * Composition root for scanner.html — the phone side of the phone/computer
 * split architecture. Deliberately thin: no OpenCV, no WebGL2, no tiled
 * rendering. It only captures full-resolution frames and ships them to the
 * host over a PeerJS DataConnection; all processing happens on the
 * computer. Any number of phones can run this page against the same host
 * at once (pooled capture) — see HostApp.js for the session-sharing logic.
 */
class ScannerApp {
  #stateMachine;
  #ui;
  ...
  async start() {
    ...
    this.#ui = new ScannerUIController({
